...
class ModelWorker:
    ...
    def generate_stream(self, params):
        ...
        print("SHITSHITSHITSHITSHIT")
                # —— 一次性生成所有 token id 并统一 decode 输出 —— 
        
        # stopping_criteria = KeywordsStoppingCriteria(keywords, tokenizer, input_ids)
        streamer = TextIteratorStreamer(tokenizer, skip_prompt=True, skip_special_tokens=True, timeout=15)
        # —— 无需新 class，直接给现有 streamer 打补丁，收集每个原始 token id —— 
        streamer.token_ids = []
        _orig_put = streamer.put
        def _patched_put(input_ids):
            # 只收集真正的单个 token id
            if input_ids.numel() > 0:
                streamer.token_ids.append(int(input_ids.item()))
            return _orig_put(input_ids)
        streamer.put = _patched_put        # 在全部 chunk 输出完后打印 raw token id 列表
        ...
        print("RAW TOKEN IDSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS:", streamer.token_ids)
