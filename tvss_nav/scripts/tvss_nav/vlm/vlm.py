...
#         self.query_sub = roslibpy.Topic(self.ros, '/user_query', 'std_msgs/String')
#         self.query_sub.subscribe(self.user_query_callback)
#         log("Subscribing to user_query topic: /user_query")
...
#                         log("Processing tool call: segment_social_entities_from_name")
...
#                         # # 如果检测到 person，就把它的 cost_value 强制设到 50 (≤50)
...
#                         # # （可选）对其他对象仍保持原来工具返回的配置
#                         # # 如果需要，也可以在这里 override 其它类别

#                         # try:
#                         #     # 发布 cost_attributes 到 ROS
#                         #     cost_json = json.dumps(cost_attrs)
#                         #     self.cost_attr_publisher.publish(roslibpy.Message({'data': cost_json}))
#                         #     log(f"[INFO] Published cost attributes for {len(cost_attrs)} object(s)")
#                         # except Exception as e:
#                         #     log(f"[ERROR] Failed to publish cost attributes: {e}")
...
#                         rospy.loginfo("Processing tool call: update_sfm_param")
#                         params = tool_call['args']
...
#     #         self.shutdown_callback()
#     #         log_fp.close()
#     def user_query_callback(self, msg: Dict[str, Any]):
#         query = msg['data']
#         log(f"[VLM] Received user_query: {query!r}")
#         with self.image_lock:
#             if not self.received_first_image:
#                 log("[VLM] No image yet, skipping this query")
#                 return
#             image_msg = self.latest_image

#         img_b64 = self.process_image(image_msg)
#         threading.Thread(
#             target=self.query_gpt,
#             args=(img_b64, query),
#             daemon=True
#         ).start()
#         self.query_counter += 1
...
#     vlm = VLM(debug=False)
#     log("VLM node is up — waiting for /user_query messages …")
#     try:
#         while not vlm.shutdown_flag:
#             time.sleep(1.0)
#     except KeyboardInterrupt:
#         vlm.signal_handler(None, None)
