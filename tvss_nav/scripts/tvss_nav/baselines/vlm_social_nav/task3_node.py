...
class VLMCostScorer:
    def __init__(self, api_key, wl=0.7, wa=1.0):
        ...
        self.spd_to_v = {"slow down": 0.10, "constant": 0.30, "speed up": 0.50, "stop": 0.00}

        self.prompt_template = (
            "You are a hospital mobile robot. Your objective is to LEAVE PATIENT ROOMS or WARD AREAS "
            "and NAVIGATE TOWARDS PUBLIC AREAS (e.g., lobby, open junctions).\n"
            "Safety rules:\n"
            "- Avoid entering or staying inside patient rooms, wards, or areas with beds/curtains.\n"
            "- Prefer open spaces; move toward lobbies, wide corridors, or junctions.\n"
            "- Treat ANY rectangular opening between walls/partitions as a PORTAL, even if there is no door leaf.\n"
            "- If you see signs/doors like 'Ward', beds, patient rooms, curtains, or many pedestrians, move away.\n"
            "- Keep low speed; stop if unsure.\n\n"
            "Answer Format:\n"
    ...
    def parse_action(self, text):
        ...
        w = {"left": +0.15, "straight": 0.0, "right": -0.15}.get(direction, 0.0)
        return v, w
...
class PublicAreaNavNode:
    def __init__(self):
        ...
        self.gamma = float(self.cfg.get("gamma", 1.5))   # VLM一致性 0.7
        ...
        print(f"[Node] VLM-Go-To-Pubulic-Area ready. Publishing {self.cmd_vel_topic}")
