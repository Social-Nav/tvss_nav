...
{
  Ped::Tvector totalForce(0, 0);
  const double cutoffDistance = 6.0; // 6 meters cutoff
  ...
  for (const Tobstacle *obstacle : scene->obstacles)
  {
    ...
    double distance = diff.length() - agentRadius;
    
    // Only consider obstacles within cutoff distance
    if (distance < cutoffDistance) {
      if (distance <= 0.0) {
        // If inside or exactly at obstacle surface, use a large constant force
        totalForce += diff.normalized() * 10.0;
      } else {
        // Use exponential decay for force magnitude
        double forceAmount = exp(-distance / forceSigmaObstacle);
        totalForce += diff.normalized() * forceAmount;
      }
    }
  }
